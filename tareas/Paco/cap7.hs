...
of natural numbers, and the predicates: E(x) ≡ x is even, O(x) ≡ x is odd

• There is an even number = ∃x. E(x) 
• Every number is either even or odd = ∀x. E(x) ∨ O(x)
• No number is both even and odd = ∀x. ¬(E(x) ∧ O(x))
• The sum of two odd numbers is even = ∀x.∀y. (O(x) ∧ O(y)) -> E(x+y)
• The sum of an odd number and an even number is odd = ∀x.∀y. (O(x) ∧ E(y)) -> O(x+y)
-}
...
• ∀x. (∃ y. wantsToDanceWith (x, y) = Todo el mundo tiene alguien con quien quiere bailar
• ∃x. (∀ y. wantsToPhone (y, x) = Hay alguien con quien todos quieren hablar
• ∃x. (tired (x) ∧ ∀y. helpsMoveHouse (x, y) = Existe alguien que esta cansado y ayuda a todos a mudarse
...
forall [1,2,3] (== 2) ∀x. x==2 =
       = (1==2) ∧ (2==2) ∧ (3==2)
       = F ∧ T ∧ F
       = F

forall [1,2,3] (< 4) ∀x. x<4 =
       = (1<4) ∧ (2<4) ∧ (3<4)
       = T ∧ T ∧ T
       = T
-}
...
exists [0,1,2] (== 2) ∃x ∈ {1,2,3}. x==2 =
       = (1==2) ∨ (2==2) ∨ (3==2)
       = F ∨ T ∨ F
       = T
exists [1,2,3] (> 5) ∃x ∈ {1,2,3}. x>5 =
       = (1>5) ∨ (2>5) ∨ (3>5)
       = F ∨ F ∨ F
       = F
...
-}
infun_8_1 :: Int -> Bool
infun_8_1 x = exists [1,2] (==x+1)  

infun_8_2 :: Int -> Bool
infun_8_2 x = forall [1,2] (==x+1)  

ex8_a = forall [1,2] infun_8_1
ex8_b = exists [1,2] infun_8_1
ex8_c = exists [1,2] infun_8_2
ex8_d = forall [1,2] infun_8_2
