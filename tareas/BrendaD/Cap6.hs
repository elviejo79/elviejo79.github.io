...
ex2b= True \/ (not True)
ex2c= not (False \/ True)
ex2d= (not (False /\ True)) \/ False
ex2e= (not True) ==> True
ex2f= True \/ False ==> True
ex2g= True ==> (True /\ False)
ex2h= False ==> False
ex2i= (not False) <=> True
ex2j= True <=> (False ==> False)
ex2k= False <=> (True /\ (False ==> True))
ex2l= (not (True \/ False)) <=> False /\ True
...
{-Exercise 4. Verify that the expression is a WFF, Build their truth table and say if its  tautology, contradiction or satisfiable.
