...
-- Exercise 3. Express the following statements formally, using the universe of natural numbers, and the predicates E(x) ≡ x is even and O(x) ≡ x is odd.
-- •There is an even number. • Every number is either even or odd. • No number is both even and odd. • The sum of two odd numbers is even. • The sum of an odd number and an even number is odd.
...
--Exe5
-- ∀ x (∃ y. espero que baile con (x,y)) 	-> -- para todas las x, existe almenos una y que espera bailar en parejas-
-- ∃ x (∀ y. wantsToPhone (y, x))			-> -- Para todas las personas y, esperan hablar en pareja, con almenos una X.
-- ∃ x (tired (x) ∧ ∀ y. helpsMoveHouse (x, y))	-> -- hay alguien cansado de ayudarles a ayudarles a mover la casa.
